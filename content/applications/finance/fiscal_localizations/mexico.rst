...
and click the :guilabel:`COA SAT (XML)` button on the :guilabel:`PDF` menu
to generate an XML file containing all of your accounts, which will be ready to upload to the |SAT|
website.
...
This report can be generated monthly with its XML file in
:menuselection:`Accounting --> Reporting --> Trial Balance`. In the calendar, select the month you
want to download, then select :guilabel:`SAT (XML)` from the :guilabel:`PDF` menu.
...
To generate this XML document, go to :menuselection:`Accounting --> Accounting -->
Journal Entries`, and create a new entry for all the amounts to be changed, balancing the debit
and/or credit of each one. When this is done, select the :guilabel:`Month 13 Closing` option
(located in the :guilabel:`Other Info` tab, in the :guilabel:`Mexico` section).
The report with the total amount of the year and all the additions of the journal entry can be found
in :menuselection:`Accounting --> Reporting --> Trial Balance`, selecting :guilabel:`Month 13` in
the calendar.

The XML file is created by selecting :guilabel:`SAT (XML)` from the :guilabel:`PDF` menu.
...
To create the XML, go to :menuselection:`Accounting --> Reporting --> General Ledger`, and select
:guilabel:`XML (Polizas)` in the :guilabel:`PDF` menu. In the
:guilabel:`XML Polizas Export Options` window, you can choose between four different
...
To get the |DIOT| report, go to :menuselection:`Accounting --> Reporting --> Tax Reports`. Select
the month in the calendar, and click :guilabel:`Report: DIOT (MX)`. To download the :file:`.txt`
file, select :guilabel:`DIOT (txt)` from the :guilabel:`PDF` menu.
